- 原理：数据类型实现了`IntoIterator`，即可作为迭代器迭代。迭代器实现了`Iterator`,`for`循环就是不断调用`next`方法，取得`Option<T>`包装的元素
- 先将**集合**转换成**迭代器**，**迭代器**拥有各种迭代方法
> [!NOTE] Tip
> 迭代器就是一个书签，不包含原始的数据的内容，只能读写甚至是消费掉
> 
```rust
pub trait Iterator {
    type Item;
...
> [!NOTE] Tip
> 通过三种不同的方法拿到不同能力的“书签”，`into_iter` 可以对原始的书进行读写且把页撕掉，`iter`只能看，`iter_mut` 可以读写不能撕页
> 

| 迭代器类型       | 获取方式  | next方法         |
| ----------- | ----- | -------------- |
| `into_iter` | 夺走所有权 | `Some(T)`      |
| `iter`      | 不可变借用 | `Some(&T)`     |
| `iter_mut`  | 可变借用  | `Some(&mut T)` |

> [!NOTE] `iterator`和`intoiterator`的区别
...
> [!NOTE] Tip
> 进行`next`遍历，将每个数据都经过迭代器适配器，最后得到的数据（比如i32等单个数据）选择性的加入其他容器等
- 将**迭代器**适配器消费掉，依赖`next`来消费元素，返回值，比如`vec`等
- **注意**：消费性迭代器通常与一些方法结合使用，比如 `sum()`、`collect()`、`for_each()` 等
- 这些方法会消耗迭代器，导致原始集合失效
- 要在迭代后继续使用原始数据，可以考虑使用引用迭代器（如 `iter()`）
- 消费者适配器：某个方法 `A` 在其内部调用了 `next` 方法，那么 `A` 就被称为消费性适配器`sum`
# 迭代器适配器

> [!NOTE] Tip
> 接受一个书签(迭代器，有能力对源数据做一些事)，定义操作内容（实际上没有立刻做，而是等后面调用next方法），返回迭代器（书签）

- 接受**迭代器**，进行操作后返回**迭代器**，例如`map,filter`
